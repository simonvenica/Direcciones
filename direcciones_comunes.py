...
class direccion:
	...
	def limpiar_texto(self,texto): #Limpia un texto de imperfecciones recurrentes y lo devuelve en minusculas
		texto = texto.lower()
		texto = texto.strip(" ")
		texto = texto.strip("\n")
		while "  " in texto:
...
def proceso_direcciones():
	texto_direccion = raw_input("Direccion a separar: ")
	direccion = direccion(texto_direccion)
	direccion = direccion.limpiar_texto(direccion)
	
	altura = buscar_altura(lista_direccion)
	if altura == 0:
		direccion = intentar_corregir_direccion(direccion)
		liosta_direccion = direccion.split(" ")
		altura = buscar_altura(lista_direccion)
